...
<body>
 	...
 	<h2>Auction Bidding Portal</h2><hr>
	...
	while (result.next()) {
	    ...
	    out.println("<input type='hidden' name='clickedFile' value='PlaceBid.jsp'>");
	...
	result = stmt.executeQuery("SELECT * FROM Bidding b JOIN Auction a on b.auctionID = a.auctioniD JOIN Item i on a.itemiD = i.itemID WHERE userName='" + session.getAttribute("user").toString() + "' ORDER BY b.auctionID DESC, b.bidTiming DESC");
	while (result.next()) {
	    out.println("<tr><td style='border: 1px solid #dddddd; padding: 8px;'>");
	    out.print(result.getString("name"));
	    out.println("</td><td style='border: 1px solid #dddddd; padding: 8px;'>");
	    out.print(result.getString("type"));
	    out.println("</td><td style='border: 1px solid #dddddd; padding: 8px;'>");
	    out.print(result.getString("auctionID"));
	    out.println("</td><td style='border: 1px solid #dddddd; padding: 8px;'>");
	    out.print(result.getString("bidTiming"));
	    out.println("</td><td style='border: 1px solid #dddddd; padding: 8px;'>");
	...
	out.println("<form action='VerifyBid.jsp' method='POST'>");
